...
We are currently using postgres. You need to set up a user,
   ...
   * create a database, a user and a role
    ```
    CREATE DATABASE quitbot_db;
    CREATE USER quitbot_user WITH PASSWORD 'quitbot_pass';
    ALTER ROLE quitbot_user SET client_encoding TO 'utf8';
    GRANT ALL PRIVILEGES ON DATABASE quitbot_db TO quitbot_user;
    ALTER ROLE quitbot_user CREATEDB;
   ```
   * If PostgreSQL version is 15+
   ```
   \c quitbot_db
   GRANT ALL ON SCHEMA public TO quitbot_user;
   ```
